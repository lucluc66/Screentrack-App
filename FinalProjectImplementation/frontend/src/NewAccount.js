...
class NewAccount extends React.Component {
  ...
  postData(event){
    ...
    fetch('http://localhost:3001/credentials/', options).then();
    alert("Submitted, Please Login With the Registered Credentials.");
  }
  ...
  render() {
    if (this.state.about) {
      return (
        <div>
          <About logined={this.state.logined}/>
        </div>
      );
    }
    if (this.state.tips) {
      return (
        <div>
          <Tips logined={this.state.logined}/>
        </div>
      );
    }
    if (this.state.contact) {
      return (
        <div>
          <Contact logined={this.state.logined}/>
        </div>
      );
    }
